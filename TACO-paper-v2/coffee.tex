...
discretized domain. With such kernel and working set structures, traditional transformations for cache locality (e.g. blocking) are not helpful; instead we focus on aspects like the minimization of floating-point operations, register locality
and instruction-level parallelism, especially vectorization.
...
performed; this is not the case for the Helmholtz
example, though. Nevertheless the performance gain from improved register reuse 
can still be greater if suitable heuristics are used. Our approach 
consists of traversing the expression tree and recursively splitting it into 
multiple sub-expressions if the number of innermost-loop independent 
variables is larger than a certain threshold. This is elaborated in the next section, 
and validated against empirical search in Section~\ref{sec:perf-results-split}.
...
the element matrix, grows proportionately with the complexity of the
...
calls to external linear algebra libraries. As suggested by the performance evaluation, an analysis of more sophisticated approaches for expression splitting also looks promising. COFFEE supports all of the
