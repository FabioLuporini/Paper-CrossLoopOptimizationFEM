...
We begin observing that our strategic choice was to develop COFFEE as a separate, self-contained software module, with clear input/output interfaces, rather then incorporating it within PyOP2. This is because there are neither conceptual nor technical reasons for which the compiler could not be used to optimize codes different than those produced by Firedrake. It would be quite easy, for example, to integrate COFFEE with the popular FEniCS framework, the pioneer of automated code generation for finite element local assembly. More challenging is to assess the generality of our optimization strategy; that is, if and how COFFEE and its transformations are transferable to other computational domains, perhaps other DSLs, and to what extent it would be helpful for improving full-application performance. To answer these questions, we need to go back to the origins of our compiler.
...
\item Compiler auto-vectorization has become effective for a variety of codes. Nevertheless, multi-loop vectorization, which involves loading and storing data along a subset of the loops characterizing the iteration space (rather than just along the innermost loop), is not supported by available general-purpose and polyhedral compilers to the best of our knowledge. However, the outer-product vectorization technique presented in this paper shows that two-loop vectorization can outperform standard auto-vectorization; more importantly, we expect the performance gain to scale with the number of vectorized loops and the vector length (as proved by the Xeon Phi results). Despite being not straightforward to automate multi-loop vectorization in a general-purpose compiler, especially if stencils are present, we believe that in specific domains this could be more easily achieved. The intuition is to map the memory access pattern on vector registers, and then relying on in-register shuffling to minimize the traffic between memory and processor.
\end{enumerate}

We conclude highlighting the fact that, from the perspective of the domain-specific language and compiler designers, our optimization strategy represents an incentive to produce extremely simple representations of the code (e.g. fully-inlined mathematical expressions in the form of an abstract syntax tree) so as to make the architecture-aware code optimizer completely responsible for choosing and applying the most suitable set of transformations. This aspect is of crucial importance since it allows achieving \textit{separation of concerns}, with domain specialists that can completely abstract from the performance optimization problem.
