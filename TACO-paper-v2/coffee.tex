...
\item The quantities in the local assembly expression are known at code generation time. This means that ``good'' code can be generated, for example with explicit loop bounds, statically-initialized constant n-dimensional arrays, and no pointer aliasing. All of these factors contribute to the effectiveness of source-to-source code transformations: by knowing iteration spaces in advance and having a clean memory access pattern, transformations such as padding, data alignment, and vector-register tiling can be automated by a compiler like COFFEE.
