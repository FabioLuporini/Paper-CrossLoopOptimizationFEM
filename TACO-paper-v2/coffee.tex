...
\begin{enumerate}
\item Expression manipulation at the mathematical layer (e.g. simplification, decomposition into sub-expressions) opens multiple semantically-equivalent code generation opportunities, characterized by different trade-offs in parallelism, redundant computation, and data locality. The basic idea is to exploit properties of arithmetic operators, like associativity and commutativity, to re-schedule the computation suitably for the underlying architecture. Loop-invariant code motion and expression splitting follow this principle, so they can theoretically be re-adapted or extended to domains sharing some of the computational characteristics of local assembly, like those involving tensor contractions~\cite{modeldriven}.
\item In the expression above, the summations correspond to reduction loops, whereas loops over indices $i$ and $j$ are fully parallel. The various sub-expressions, such as $a$, $b$, and $c$, in general only depend on (i.e. iterate along) a subset of the enclosing loops. In addition, some of these sub-expressions might reduce to the same values as iterating along certain iteration spaces. This code structure lies at the core of generalized loop-invariant code motion. Whenever sub-expressions invariant with respect to different sets of loops can be identified, the question of whether, where and how to hoist them, while minimizing redundant computation, arises. Any codes presenting these characteristics is amenable to this kind of transformation. Note that, in general, for applying our technique the loop nest could be arbitrarily complex (e.g. more loops), provided that it is affine.
\item TODO expression splitting -> key to loop/kernel fission
