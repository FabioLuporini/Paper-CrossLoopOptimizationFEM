...
We first notice that, in the scenario $f_1p_1$, the assembly is dominated by matrix insertion: despite the application of generalized loop-invariant code motion, data alignment and padding, there is only a minimal performance gain. This changes instantly moving to $p=2$ and $f=2$. In these scenarios, not only the cost of \texttt{assembly} becomes larger with respect to \texttt{solve}, but also does it make insertion cost negligible. The transformations automatically applied by COFFEE successfully decrease the impact of \texttt{assembly} over \texttt{solve}: this was achieved by a combined application of generalized loop-invariant code motion, alignment and padding, and expression splitting. Interesting is the fact that generalized loop-invariant code motion was particularly intensive, with 23 temporaries generated and several redundancies discovered (see Section~\ref{sec:code-transf-licm}). 
