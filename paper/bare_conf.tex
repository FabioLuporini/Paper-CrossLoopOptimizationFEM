...
\begin{algorithm}[t]
\small
\caption{Local assembly code generated by FFC for a Helmholtz problem (2D mesh, Lagrange $p=1$ elements). The padding, data alignment, $licm$ and $split$ optimizations are applied. Data alignment and padding relate to an AVX machine. In this specific case, the split factor is 2.}
...
\begin{algorithm}[t]
\small
\caption{Local assembly code generated by FFC for a Helmholtz problem (2D mesh, Lagrange $p=2$ elements). The padding, data alignment, $licm$ and $op$-$vect$ optimizations are applied. Data alignment and padding relate to an AVX machine. In this specific case, the unroll-and-jam factor is 1.}
...
\end{tabulary}
\caption{Overview of code transformations for FFC-generated assembly kernels.}
...
Our approach to solving partial differential equations is based on domain-specific languages and libraries. Fenics and Firedrake provide the high-level Unified Form Language (UFL) to write problems in a notation resembling mathematical equations (``forms''). In Fenics, this specification is translated by a form compiler (FFC) into C code, which is subsequently just-in-time compiled and executed on the target architecture. In Firedrake we adopt a completely different approach. We think that FFC should produce a high-level representation of a Finite Element assembly kernel, and should abstract from the specific details of the underlying architecture. We have modified FFC, therefore, to build Abstract Syntax Trees (AST), rather than C code. ASTs are then passed to PyOP2, which implements the core of Firedrake, where parallel execution over the unstructured mesh is managed. We have developed a compiler that, given an AST conformed to the PyOP2 specification (such as those produced by the new FFC), can apply the transformations described in Section~\ref{sec:code-transf} and, subsequently, generate C code. The C representation is eventually just-in-time compiled for one of the backends that are currently supported by PyOP2, namely OpenMP, MPI, OpenMP+MPI, CUDA, OpenCL. Our compiler is now fully integrated with PyOP2, so it supports any kind of problem expressible in UFL. 

The PyOP2 compiler has been thought to support a range of higher-level languages and applications that concern the resolution of partial differential equations; Firedrake, in terms of its components UFL and FFC, is one of them. We believe, however, that many of the discussed optimizations are generalizable and applicable to non-Firedrake kernels. For example, it is always the case that Finite Element Local Assembly using a quadrature strategy leads to a (more or less complex) loop nest and outer-product-like computations. 

The compiler structure is outlined in Figure~\ref{algo:PyOP2Compiler}. The AST is initially inspected, looking for the presence of an iteration space and other domain-specific information provided by the upper layer. If the kernel is just a sequence of statement, then so-called inter-kernel vectorization, in which a non-affine loop over mesh elements is vectorized, can be applied. This might be useful in a Finite Volume context, where kernels often lack an iteration space representing the bulk of the computation. Although not implemented yet, Reguly et. al. already proved the profitability of this technique in the OP2 framework~\cite{reguly-inter-kernel}. If, instead, an iteration space is found and the underlying platform is a GPU, the compiler seeks fully-parallel loops (i.e. loops whose iterations can be executed in any order without need for synchronization) and hoist them out at the level of the ``wrapper'' (i.e. the function that iterates over mesh elements and invokes the kernel); this is aimed at optimizing register management on GPUs. 

If the backend is a CPU and the AST comes from Firedrake, then a sequence of optimization steps are executed. A cost model, currently under development, is queried to estimate the potential gain (or loss) coming from the $split$ and $op$-$vect$ transformations. The model suggests a split factor and a tile size. Firstly, the compiler applies $licm$, which introduces temporary arrays, and on top of this padding and data alignment are enforced. These three transformations are always applied, since they usually provide a significant performance improvement (this will be shown in Section~\ref{sec:perf-results}). Finally, $split$ and/or $op$-$vect$ take place.  
