...
The complexity of FFC-generated kernels depends on the mathematical problem being solved. In simpler cases, the loop nest (always characterized by affine memory accesses) is perfect, it has short trip counts (in the range 3-15), and the computation reduces to a summation of a few, small outer-product vector-operations. An example is provided in Figure~\ref{helmholtz-code}, which shows an assembly kernel for a Helmholtz problem, using Lagrange basis functions on 2D elements with polynomial order $p=1$. In other situations, for instance when solving a non-linear problem like Burgers (see Figure~\ref{burgers-code}), the number of arrays involved in the computation of the stiffness matrix can be much larger (14 in the case of Burgers using $p=1$ Lagrange tetrahedrons). Also, constants evaluated in outer loops, acting as scaling factors of arrays, may be required; trip counts can be larger (the increment is proportional to the order of the method); arrays may be block-sparse. Despite such a big space of possible assembly implementations, it is still possible to identify a domain-specific structure common to all kernels, which can be turned into effective code transformations and simd-vectorization.
