...
place. The flow is summarized in Figure~\ref{fig:coffee-pipeline}.
...
choice also simplifies COFFEE's cost model. The only situation in
...
\item Burgers
\end{enumerate}
The three chosen benchmarks comprise the core differential operators in
some of the most frequently encountered finite element problems in
scientific computing. The Helmholtz and Diffusion kernels are archetypal
second order elliptic operators. They are complete and unsimplified
examples of the operators used to model diffusion and viscosity in
fluids, and for imposing pressure in compressible fluids. As such, they
are both extensively used in climate and ocean modeling. Very similar
operators, for which the same optimisations are expected to be equally
effective, apply to elasticity problems, which are a foundation of
...
momentum itself, in our case). We chose this particular kernel since it
applies to a vector-valued quantity, while the elliptic operators apply
to scalar quantities. This impacts the generated code, as explained
next. The operators we have selected are characteristic of both the
second and first order operators that dominate simulations in fluids and
solids, and a wide variety of local assembly codes can be tested.

The benchmarks, available at~\cite{ufl-code}, were written in UFL
...
nonlinear problem employing differential operators different from
...
family, Lagrange, is fixed. As expected, the larger the
...
consequence, the size of the $X$ and $Y$ arrays). A range of polynomial
...
% FIXME: weren't those FIAT limitations?
...
Section~\ref{sec:pyop2-compiler}, loop interchanges having an inner loop
along \texttt{i} caused slow downs; also, interchanging \texttt{j} and
\texttt{k} loops while keeping \texttt{i} as outermost loop did not
...
usually benefit from the reduction in redundant computation, even though
extra memory for the temporary arrays is required.  Exceptions to this
trend are due to a less effective auto-vectorization. For instance, on
the Sandy Bridge, the improvement at $p=3$ is larger than that at $p=4$
because, in the latter case, the size of the innermost loop is not a
multiple of the vector length, and a remainder scalar loop is introduced
at compile time. Since the loop nest is small, the cost of executing the
extra scalar iterations can have a significant impact. The remainder
loop overhead is more pronounced on the Xeon Phi, where the vector
length is twice as big, which leads to proportionally larger scalar
remainder loops.
...
\emph{licm}, data alignment, and padding over the original code. In the
following, this version of the code is referred to as \emph{licm-ap}.
Padding, which avoids the introduction of a remainder loop as described
in Section~\ref{licm-impact}, as well as data alignment, enhance the
quality of auto-vectorization. Occasionally the run-time of
\emph{licm-ap} is close to that of \emph{licm}, since the non-padded
element matrix size is already a multiple of the vector length. In rare
cases \emph{licm-ap} is slower than \emph{licm} (e.g. in Burgers $p=3$
on the Sandy Bridge). One possible explanation is that the number of
aligned temporaries introduced by \emph{licm} is so large to induce
...
It is worth noting that, in most cases, the cost model successfully
determines how to transform a kernel to maximize its performance. This
is chiefly because assembly kernels fit the L1 cache, such that, with a
...
improved register re-use. In Helmholtz $p=4$, for example, the tool
...
matrix evaluation needs to be read from a large number of basis function
arrays. As detailed in Section~\ref{sec:expr-split}, the price to pay
for this optimazion is an increased number of accesses to the element
matrix and, potentially, redundant computation. Similarly to the
analysis of vector-register tiling, we compare two versions: the
baseline, \emph{licm-ap}; and expression splitting on top of
\emph{licm-ap}, which, for simplicity, is referred to as \emph{split} in
the following.
...
slow down the computation, because of the aforementioned drawbacks. Slow
downs up to 1.4$\times$ and up to 1.6$\times$ respectively were observed
on the Sandy Bridge and the Xeon Phi. Note that the cost model prevents
the adoption of the transformation: the \texttt{while} statement in
Figure~\ref{algo:applyCostModel} is indeed never entered.
...
improvement over the original code increases from 1.44$\times$ to
2.11$\times$. On the Xeon Phi, the impact of \emph{split} is only
marginal, since register spilling is limited by the presence of 32
logical vector units.
...
\emph{split}. This is explained by two factors: for small split
factors, such as 1 and 2, the data space to be tiled is still too big,
and register spilling affects run-time. For higher factors,
...
syntax tree representation of a local assembly kernel, rather than plain
C++ code to enable code transformations in COFFEE. In the FEniCS
project~\cite{Fenics}, FFC's role is generating an optimized C++ code
string confirming to the UFC interface. The model-driven optimizations
to local assembly kernels~\cite{quadrature1} supported by FFC mainly
consist of loop-invariant code motion and elimination, at code
...
operation counts may not decrease significantly.
...
kernels. Slow downs in the Burgers benchmark occur in presence of a
small iteration space (triangular mesh, $p \in [1, 2]$; tetrahedral
mesh, $p \in [1, 2]$; prismatic mesh, $p = 1$). The result shown
represents the worst slow down, which was obtained with a triangular
mesh and $p = 1$. This is a result of removing zero-valued entries in
basis function arrays: some operations are avoided, but indirection
arrays prevent auto-vectorization, which significantly impacts
performance as soon as the element matrix becomes bigger than
12$\times$12. However, with the forthcoming zero-removal optimization in
COFFEE, we expect to outperform FEniCS in all problems. In the cases of
Helmholtz and Diffusion, the minimum improvements are, respectively,
1.10$\times$ and 1.18$\times$ (2D mesh, $p=1$), which tend to increase
with polynomial order and element shape up to the values illustrated in
the table.
...
Fluidity~\cite{fluidity_manual_v4}. Numerical integration is usually
employed to implement the local assembly phase. The recent introduction
of domain-specific languages (DSLs) to decouple the finite element
specification from its underlying implementation facilitated the
development of novel approaches. Methods based on tensor
contraction~\cite{FFC-Compiler} and symbolic manipulation~\cite{Francis}
have been implemented. We have designed COFFEE to specifically target
numerical integration because it has been demonstrated that it remains
the optimal choice for a wide class of problems~\cite{quadrature1}.

Optimization of local assembly using numerical integration for CPU
platforms has been addressed in FEniCS~\cite{quadrature1} and a
comparison to COFFEE is presented in
Section~\ref{sec:perf-results-fenics}. In~\cite{Markall20101815}, and
more recently in~\cite{petsc-integration-gpu}, numerical integration has
been studied for GPU architectures. In~\cite{assembly-opencl}, variants
of the standard numerical integration algorithm have been specialized
and evaluated for the PowerXCell processor, but an exhaustive study from
the compiler viewpoint --- like ours --- is missing, and none of the
optimizations presented in Section~\ref{sec:code-transf} are mentioned.
Among these efforts, to the best of our knowledge, COFFEE is the first
work targeting low-level optimizations through a real compiler approach.
...
Spiral~\cite{Pueschel:05} for digital signal processing algorithms,
~\cite{Spampinato:14} for dense linear algebra, or
...
codes. COFFEE improves the efficiency of automatically generated kernels
in Firedrake.
