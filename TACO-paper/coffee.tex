...
The time required to apply the numerical kernels is a major issue, since the equation domain needs to be discretized into an extremely large number of elements, possibly order of trillions \cite{Rossinelli2013}, in order to obtain a satisfactory approximation of the PDE. For example, it has been well established that mesh resolution is critical in the accuracy of numerical weather forecasts. However, operational forecast centres have a strict time limit in which to produce a forecast (60 minutes in the case of the UK Met Office, who are partially funding this work), so producing efficient kernels has a direct scientific payoff in higher resolution, and therefore more accurate, forecasts. In finite elements, numerical kernels are executed during the local assembly phase (or ``assembly'', in the following), which can cover a significant fraction of the overall computation run-time, often in the range 30-60$\%$. In this paper, we aim at improving the performance of local assembly kernels in a wide variety of notably important PDEs though aggressive code transformation.
